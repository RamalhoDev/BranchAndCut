...
// Inicializador
Data::Data(int qtParam, char *instance) : xCoord(NULL), yCoord(NULL) {
	if (qtParam < 2) {
...
void Data::readData() {
	...
	// Alocar matriz 2D
	distMatrix = vector<vector<double>>(dimension, vector<double>(dimension, 0));  // memoria dinâmica (matrix 2D)
...
double Data::CalcDistEuc(double *X, double *Y, int I, int J) { return sqrt(pow(X[I] - X[J], 2) + pow(Y[I] - Y[J], 2)); }
